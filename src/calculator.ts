...
const functions = {
    ...
    extraOn: () => {
        ...
        $("div.btn[alt-text]").each((_, element) => {
            $(element.children[0]).text(<string>$(element).attr('alt-text'))
        })
    },
    extraOff: () => {
        isAltMode = false;
        $("div.btn[alt-text]").each((_, element) => {
            $(element.children[0]).text(<string>$(element).attr('text'))
        })
    },
    random: () => {
...
let shouldMultiply = (eq: string, write: string) => {
...
$("div.btn").click((event) => {
    ...
    const altUse = element.attr('alt-use');
    
    if (isAltMode && element.attr('alt-write')) {
        if (shouldMultiply(equation, <string>element.attr('alt-write'))) {
            equation += '×';
        }
        ...
        if (shouldMultiply(equation, <string>element.attr('write'))) {
            equation += '×';
        }
